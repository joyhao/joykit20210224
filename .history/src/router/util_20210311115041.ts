...
export function authRouter(): RouteRecordRaw[] {
  ...
  arr.map(([key, value], index) => {
    ...
    const child = getChildren(key, value);
    os.children.push(child as never);
    list[index] = os;
  });
...
type childRoute = RouteRecordRaw | null;
function getChildren(key: string, value: string): childRoute {
